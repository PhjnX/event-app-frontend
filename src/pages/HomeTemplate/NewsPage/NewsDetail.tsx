import { useEffect, useState, useRef, type JSX, useMemo } from "react";
...
const styles = `
  ...
  .page-light { background: var(--page-bg); min-height: 100vh; color: var(--text); }
  ...
  .article-body p { font-size: 17.5px; line-height: 2; margin-bottom: 1.7rem; color: var(--text-body); text-align: justify; }
  .article-body h2 {
  ...
  .article-body h3 { font-size: 1.05rem; font-weight: 800; color: var(--text); margin: 2.5rem 0 0.6rem; letter-spacing: -0.01em; display: block; }
  ...
  .img-block-frame::before, .img-block-frame::after { content: ''; position: absolute; left: 0; right: 0; z-index: 2; height: 1px; background: linear-gradient(90deg, transparent 0%, var(--gold-border) 20%, rgba(184,150,12,0.45) 50%, var(--gold-border) 80%, transparent 100%); pointer-events: none; }
  ...
  .dq { position: relative; margin: 2.5rem 0; padding: 2rem 2rem 2rem 2.5rem; background: linear-gradient(135deg, rgba(184,150,12,0.055) 0%, rgba(184,150,12,0.01) 100%); border-left: 3px solid var(--gold); border-radius: 0 14px 14px 0; border-top: 1px solid rgba(184,150,12,0.12); border-bottom: 1px solid rgba(184,150,12,0.12); border-right: 1px solid var(--border); }
  ...
  .contact-strip { margin-top: 2.5rem; padding: 1.1rem 0 1.1rem 1.4rem; border-left: 2px solid var(--gold-border); display: flex; flex-wrap: wrap; align-items: center; gap: 0 0; }
  .contact-link { display: inline-flex; align-items: center; gap: 6px; text-decoration: none !important; transition: color 0.18s; padding: 3px 0; }
  .contact-link:hover { color: var(--gold-dark) !important; }
  .contact-sep { width: 1px; height: 14px; background: var(--border-med); margin: 0 16px; flex-shrink: 0; }

  .article-byline { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; padding: 16px 0; margin-top: 8px; border-top: 1px solid var(--border); }

  .related-bottom { margin-top: 3.5rem; padding-top: 2.5rem; border-top: 2px solid var(--border-med); }
  .related-bottom-header { display: flex; align-items: center; gap: 12px; margin-bottom: 1.75rem; }
  .related-bottom-grid { display: grid; grid-template-columns: repeat(1, 1fr); gap: 16px; }
  @media (min-width: 640px) { .related-bottom-grid { grid-template-columns: repeat(2, 1fr); } }
  @media (min-width: 1024px) { .related-bottom-grid { grid-template-columns: repeat(3, 1fr); } }
  .rel-bottom-card { background: var(--surface); border: 1px solid var(--border-med); border-radius: 14px; overflow: hidden; transition: all 0.3s cubic-bezier(0.22,1,0.36,1); text-decoration: none !important; display: flex; flex-direction: column; }
  .rel-bottom-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-lg); border-color: var(--gold-border); }
  .rel-bottom-img-wrap { position: relative; overflow: hidden; aspect-ratio: 16/9; background: var(--surface-2); }
  .rel-bottom-img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s cubic-bezier(0.22,1,0.36,1); }
  ...
  .rel-bottom-title { font-size: 14px; font-weight: 700; line-height: 1.45; color: var(--text-sub); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; transition: color 0.2s; }
  ...
  .rel-bottom-arrow { width: 26px; height: 26px; border-radius: 50%; background: var(--gold-dim); border: 1px solid var(--gold-border); display: flex; align-items: center; justify-content: center; transition: all 0.2s; flex-shrink: 0; margin-left: auto; }
  .rel-bottom-card:hover .rel-bottom-arrow { background: var(--gold); border-color: var(--gold); }
  ...
  .cat-pill:hover { background: var(--gold-dim); border-color: var(--gold-border); color: var(--gold-dark); }
`;
            ...
            {category && (
              <>
                <span className="flex items-center gap-2">
                  <Tag size={13} style={{ color: "#D4AE1A" }} />
                  <span
                    style={{
                      color: "#D4AE1A",
                      fontWeight: 700,
      ...
      {blocks.map((block: any) => {
        switch (block.type) {
          ...
          case "image": {
            ...
            const isVideo = checkIsVideo(imgSrc);
    ...
    <div className="related-bottom font-noto">
      ...
      <div className="related-bottom-grid">
        {filtered.map((post) => {
          ...
          const catName = categoryMap.get(post.categoryId) || post.categoryName;
              ...
              <div className="rel-bottom-img-wrap">
                {isVideo ? (
                  <video
                    ...
                    loop
                    playsInline
                    className="w-full h-full object-cover rel-bottom-img"
                  />
                ) : (
                  <OptimizedImage
    ...
    <div className="space-y-5 font-noto">
        ...
        <div>
          {relatedPosts
            ...
            .map((post, idx) => {
              ...
              const catName =
                categoryMap.get(post.categoryId) || post.categoryName;
              const catSlug = post.categorySlug || "tin-tuc";
              const isVideo = checkIsVideo(post.thumbnailUrl);

              return (
                <Link
                  to={`/news/${catSlug}/${post.slug || post.id}`}
                  key={post.id}
                ...
                >
                  ...
                  >
                    {isVideo ? (
                      <video
                        ...
                        loop
                        playsInline
                        className="rel-thumb w-full h-full object-cover"
                      />
                    ) : (
                      <OptimizedImage
                  ...
                  <div className="flex-1 min-w-0">
                    {catName && (
                      <span
                        className="text-[10px] font-bold uppercase tracking-wide mb-0.5 block"
                        style={{ color: "var(--gold-dark)" }}
                      >
                        {catName}
                      </span>
      ...
      <div className="ev-card aspect-3/4">
        {upcomingEvent ? (
          ...
          >
            {checkIsVideo(
              upcomingEvent.bannerImageUrl || upcomingEvent.thumbnailUrl,
            ) ? (
              <video
...
const NewsDetail = () => {
  ...
  const postCategoryName =
    (postDetail.categoryId ? categoryMap.get(postDetail.categoryId) : null) ||
    postDetail.categoryName ||
    null;
  const postCategorySlug = postDetail.categorySlug || categorySlug || "tin-tuc";

  const currentUrl = `${DOMAIN}/${currentLang}/news/${postCategorySlug}/${
    postDetail.slug || postDetail.id
  }`;
  ...
  return (
    <>
      <SeoHelmet
        ...
        slug={`news/${postCategorySlug}/${postDetail.slug || postDetail.id}`}
              ...
              <div className="article-body-wrap">
                ...
                >
                  ...
                  {postCategoryName && (
                    <>
                      <Link
                        to={`/news?category=${postCategorySlug}`}
