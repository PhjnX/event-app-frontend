 ...
 *
 * Backend dùng `LocalDateTime` cho toàn hệ thống nên chuỗi trả về không có hậu
 * tố múi giờ (ví dụ "2026-09-10T05:48:26"), nhưng server lại chạy giờ **UTC**.
 * JavaScript gặp chuỗi ISO thiếu múi giờ thì hiểu là giờ địa phương, nên nếu
 * parse thẳng sẽ lệch đúng bằng chênh lệch múi giờ của máy người dùng — ở Việt
 * Nam là 7 tiếng, khiến báo cáo vừa gửi hiện thành "7 giờ trước".
 *
 * Vì vậy: chuỗi nào chưa có múi giờ thì coi là UTC.
 */
export const parseServerDate = (iso?: string | null): Date => {
  if (!iso) return new Date(NaN);
  // Đã kèm Z hoặc +07:00 thì trình duyệt tự hiểu đúng, không đụng vào
  const coMuiGio = /(?:Z|[+-]\d{2}:?\d{2})$/.test(iso);
  return new Date(coMuiGio ? iso : `${iso}Z`);
};

/** Số giờ đã trôi qua kể từ mốc thời gian của server. */
export const hoursSinceServerDate = (iso?: string | null): number =>
  (Date.now() - parseServerDate(iso).getTime()) / 36e5;
